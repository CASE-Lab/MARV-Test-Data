...
df_ekf_nav[nav_prefix + 'vel_mag'] = np.sqrt(df_ekf_nav[nav_prefix + 'vel_x']**2 + df_ekf_nav[nav_prefix + 'vel_y']**2)
...
df_log2_tcu[tcu_prefix + 'aps'] = df_log2_tcu[tcu_prefix + 'aps_in']
df_log2_tcu[tcu_prefix + 'rps'] = df_log2_tcu[tcu_prefix + 'rps_in']
df_plt[tcu_prefix + 'aps'] = df_log2_tcu[tcu_prefix + 'aps'].resample('40ms').mean()
df_plt[tcu_prefix + 'rps'] = df_log2_tcu[tcu_prefix + 'rps'].resample('40ms').mean()
...
vel_mag_plot = df_plt.hvplot(
    title = 'Velocity Magnitude',
    kind='line',
    x = 'timestamp',
    y = nav_prefix + 'vel_mag',
    width = 1600,
...
throttle_aps_plot = df_plt.hvplot(
    title = 'Throttle APS',
    kind='line',
    x = 'timestamp',
    y = tcu_prefix + 'aps',
...
throttle_rps_plot = df_plt.hvplot(
    title = 'Throttle RPS',
    kind='line',
    x = 'timestamp',
    y = tcu_prefix + 'rps',
...
vel_mag_plot = selection_linker(vel_mag_plot)
throttle_aps_plot = selection_linker(throttle_aps_plot)
throttle_rps_plot = selection_linker(throttle_rps_plot)
...
for start_vline, stop_vline in zip(start_vlines, stop_vlines):
    vel_mag_plot = vel_mag_plot * start_vline * stop_vline
    steering_plot = steering_plot * start_vline * stop_vline
    heading_plot = heading_plot * start_vline * stop_vline
    throttle_aps_plot = throttle_aps_plot * start_vline * stop_vline
    throttle_rps_plot = throttle_rps_plot * start_vline * stop_vline
...
bokeh_server = pn.panel((geo_plot*pos_plot*start_marker_plot + vel_mag_plot + throttle_aps_plot + throttle_rps_plot + steering_plot + heading_plot).cols(1)).show(port=12345)
